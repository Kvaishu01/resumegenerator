...
def generate_resume_text(name,phno,email,linkedIn_pro,skillset,achievements,qualification):
    prompt=f"""
             create a modern and professinal resume
             Name:{name}
             phone no:{phno}
             email:{email}
...
def create_resume_pdf(name,generate_text):
    filename=f"{name.replace(' ','_')}_resume.pdf"#surendar_resume.pdf
...
def send_mail(pdf_gen,email):
    sender_email="surenc1690@gmail.com"
    password="dijq ufow hanc qtcm"
    msg=MIMEMultipart()
    ...
    with open(pdf_gen,"rb") as f:
        ...
        part['Content-Disposition']=f'attachment filename="{os.path.basenmae(pdf_path)}"'
        msg.attach(part)
    try:
...
def agentic_resume(name,phno,email,linkedIn_pro,skillset,achievements,qualification):
    generate_text=generate_resume_text(name,phno,email,linkedIn_pro,skillset,achievements,qualification)
...
with gr.Blocks() as app:
    ...
    with gr.Row():
        ...
        linkedIn_pro=gr.Textbox(label="Enter linkedin profile link")
    with gr.Row():
        achievements=gr.Textbox(label="Enter Achievments"
                       ,lines=5)
        skillset=gr.Textbox(label="Enter skillset"
                     ,lines=5)
    with gr.Row():
    ...
    gen_button.click(agentic_resume,inputs=[name,phno,email,skillset,achievements,linkedIn_pro,qualification],outputs=[output,file_output])
app.launch()
